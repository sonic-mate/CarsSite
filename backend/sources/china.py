...
async def fetch(
    ...
    **_,
    ...
    sql = f"SELECT * FROM china WHERE {' AND '.join(where)} ORDER BY AUCTION_DATE DESC LIMIT {limit} OFFSET {offset}"
...
def _norm(i: dict) -> dict:
    ...
    price_rub = int(finish * tariff_cache.get().cny_to_rub)
